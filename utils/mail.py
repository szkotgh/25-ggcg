...
def send_signup_verify_code(email, code, user_name):
    ...
    message["Subject"] = f"[스마일알러지] 이메일 확인 코드: {code}"
    message["From"] = sender_email
    message["To"] = email

    html = f'''
<h2>안녕하세요, {user_name}님.</h2>
<p>아래 이메일 확인 코드를 입력해 스마일알러지 서비스 가입을 마무리해주세요.</p><br>
...
본 이메일은 스마일알러지 서비스 가입을 위해 발송되었습니다.<br>
...
def send_welcome(email, user_name):
    message = MIMEMultipart("alternative")
    message["Subject"] = "[스마일알러지] 가입을 환영합니다"
    message["From"] = sender_email
    message["To"] = email

    html = f'''
<h2>안녕하세요, {user_name}님.</h2>
<p>스마일알러지 서비스에 가입해 주셔서 감사합니다.</p>
